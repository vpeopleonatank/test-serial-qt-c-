    ...
    , ui(new Ui::Widget)
{
    ...
    for(int i = 0 ; i < baudRates.size() ; i++){
        QString baudRateStr = QString::number(baudRates.at(i));
        stringBaudRates.append(baudRateStr);
        if (baudRateStr == "57600") {
            baudRateIdx = i;
        }
    }
    ui->comboBox_2->addItems(stringBaudRates);
    if (baudRateIdx != -1) {
        ui->comboBox_2->setCurrentIndex(baudRateIdx);
    }
...
void Widget::on_pushButton_clicked()
{
    ...
    ui->textBrowser->append(message);
    QByteArray messageByes = QByteArray::fromHex(message.toLatin1());
    serialPort.write(messageByes);
}
